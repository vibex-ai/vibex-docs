...
| `VIBEX_TLS_MODE` | `loopback_http` when `VIBEX_DEPLOYMENT_MODE=loopback`, otherwise `trusted_https_proxy` | Four values: `loopback_http` (plain HTTP on loopback only), `trusted_https_proxy` (a reverse proxy terminates TLS), `pinned_certificate` (the runtime terminates TLS with a self-signed certificate that clients pin), and `server_certificate` (the runtime terminates TLS with PEM files you provide). See [Self-hosted headless runtime](/en/self-hosted-server#environment-variables) for details. |
