...
Events carry authoritative sequence and version information. A generation or cursor gap must return `resync_required` and fetch authoritative state again. File writes use revision/CAS checks.

Remote attachments re-authenticate and authorize before a stream opens. Terminal input carries workspace scope and generation checks and is audited; do not retain raw terminal bytes.
